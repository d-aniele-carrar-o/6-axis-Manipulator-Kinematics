...
public:
    ...
    void operator()( ArgumentList outputs, ArgumentList inputs ) {
        ...
        // THETA 1 ========================================================================================
        TypedArray<double> p05 = matrixMult( Te, f.createArray<double>( {4,1}, {0, 0, -D[6], 1} ) );
        ...
        p15_1[0] = p15_1[0] - A[1];
        std::cout << "p15_1: " << p15_1[0] << ", " << p15_1[1] << ", " << p15_1[2] << std::endl;
        double l_p15             = sqrt( p15_1[2]*p15_1[2] + p15_1[0]*p15_1[0] );
        double cos_gamma         = (D[4]*D[4] + A[2]*A[2] - l_p15*l_p15) / (2*D[4]*A[2]);
        double gamma_1           = atan2( sqrt(1-cos_gamma*cos_gamma), cos_gamma );
        double sin_alpha         = D[4]*sin( gamma_1 ) / l_p15;
        double alpha             = atan2( sin_alpha, sqrt(1-sin_alpha*sin_alpha) );
        double beta              = atan2( p15_1[2], p15_1[0] );
        ...
        p15_2[0] = p15_2[0] - A[1];
        std::cout << "p15_2: " << p15_2[0] << ", " << p15_2[1] << ", " << p15_2[2] << std::endl;
        l_p15                    = sqrt( p15_1[2]*p15_1[2] + p15_1[0]*p15_1[0] );
        cos_gamma                = (D[4]*D[4] + A[2]*A[2] - l_p15*l_p15) / (2*D[4]*A[2]);
        double gamma_2           = atan2( sqrt(1-cos_gamma*cos_gamma), cos_gamma );
        sin_alpha                = D[4]*sin( gamma_2 ) / l_p15;
        alpha                    = atan2( sin_alpha, sqrt(1-sin_alpha*sin_alpha) );
        beta                     = atan2( p15_2[2], p15_2[0] );

        double th2_3             = beta - alpha;
        double th2_4             = beta + alpha;
